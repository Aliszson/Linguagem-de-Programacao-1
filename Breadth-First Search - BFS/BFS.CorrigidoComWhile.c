...
void BFS(struct Grafo *grafo, int verticeInicial, int verticeDestino)
{
    ...
    // Inicializar predecessores com -1
    for (int i = 0; i < grafo->vertices; i++)
    {
        predecessores[i] = -1;
    }
    ...
    {
        printf("Caminho do vertice %d ao vertice %d: ", verticeInicial, verticeDestino);

        int caminho[MAX_VERTICES];
        int tamanho = 0;
        int vertice = verticeDestino;
        while (vertice != -1 && vertice != verticeInicial) // Adicionada a condição vertice != verticeInicial
        {
            caminho[tamanho++] = vertice;
            vertice = predecessores[vertice];
        }

        caminho[tamanho++] = verticeInicial; // Adiciona o vértice inicial ao caminho
    ...
    while (continuar == 's' || continuar == 'S')
{
    ...
    {
        printf("O vertice de destino nao existe.\n");
    }
    else if (verticeInicial == verticeDestino)
    {
        printf("Caminho do vertice %d ao vertice %d: %d\n", verticeInicial, verticeDestino, verticeInicial);
    }
    else
    {
    ...
    printf("Deseja testar outros vertices? (s/n): ");
    scanf(" %c", &continuar);
}
